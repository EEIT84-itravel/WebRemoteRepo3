...
@WebServlet("/_05_Member/member/delforum.controller")
public class DeleteForumCollectionServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String typeId = "type_id05";
		//轉換資料的型態 
		if(temp != null && temp.trim().length()!=0){
			referenceType = Integer.parseInt(temp);
		}else{
			System.out.println("缺少討論區編號");
		}
		...
		response.sendRedirect(path+"/_05_Member/member/MemberCollectionForum.jsp");
		 return;
